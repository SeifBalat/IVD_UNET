...
from UNet import dice_coef, get_unet, dice_coef_loss
import glob
...
'''
Fixing the Warning!
The purpose of these warnings is to inform you about the built TensorFlow from source. 
 When you build the TensorFlow from the source it can be faster on the machine. 
 So all these warnings are telling you about is the build up TensorFlow from source.
 If you have a GPU on your machine, then you can ignore these warnings from AVX support. 
 Because most expensive ones will be dispatched on a GPU device.
 The Error: Your CPU supports instructions that this TensorFlow binary was not compiled to use: AVX AV
'''
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'

#Define the path of the training images, training masks and the test images
...
#printing the images type ans it's shape
...
print('Fitting Model')
print('*'*40)
results = model.fit(X_train, Y_train, batch_size=4, epochs=20, shuffle=True,validation_split=0.1,callbacks=[checkpointer])
