...
Y_train = np.zeros((len(mask_ids)*2, IMG_HEIGHT, IMG_WIDTH, 1), dtype = np.bool)
# test images
...
for n, id_ in tqdm(enumerate(test_ids), total=len(test_ids)):
    ...
    test_path = TEST_PATH + id_ + '/Opp/' +'Coronal/' 
...
for n, id_ in tqdm(enumerate(train_ids), total=len(train_ids)): 
    ...
    for folder in folder_array:
        ...
        print(train_axial_path_inn_images)
    
        train_images = next(os.walk(train_axial_path_inn_images))[2]
        ...
        for m, image_name in tqdm(enumerate(train_images), total=len(train_images)): 
            images = imread(train_axial_path_inn_images + image_name)[:,:,:IMG_CHANNELS]
...
print('Fitting Model')
print('*'*40)
history  = model.fit(X_train, Y_train, batch_size=4, epochs=150, shuffle=True,validation_split=0.1,callbacks=[checkpointer])
